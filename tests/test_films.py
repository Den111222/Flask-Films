...
'''Тесты работают, но для этого нужно закоментировать 
в src/resources/films.py @token_required. Или зарегистрироваться 
и в запросе добавить header Authorization: 'Basic <login:password in base 64>'
Тесты можно проверить для отдельного модуля "tests.test_films.TestFilms.test_get_films_with_db"
 поскольку get без проверки авторизации'''
